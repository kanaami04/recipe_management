...
async function mockApi(page: Page) {
  ...
  let sharedUser: (typeof owner)[] = []
  const body = () => ({ id: listId, owner, shared_user: sharedUser, items })
  ...
  await page.route('**/api/shopping_list/*/shares/', (route) => {
    const payload = route.request().postDataJSON() as { shared_user: { username: string }[] }
    sharedUser = payload.shared_user.map((s) => ({
      id: 'u-hanako',
      username: s.username,
      avatar_url: null,
    }))
...
test('共有ダイアログで共有相手を設定できる', async ({ page }) => {
  // Arrange
  await mockApi(page)
  await login(page)
  await page.getByRole('button', { name: '買い物リスト' }).click()

  // Act: 共有ダイアログを開いて相手を選び保存
  await page.getByRole('button', { name: '共有相手を編集' }).click()
  await page.getByRole('button', { name: '共有する相手を選択' }).click()
  await page.getByText('hanako').click()
  // 複数選択の Popover は開いたままなので、閉じてから保存する。
  await page.keyboard.press('Escape')
  await page.getByRole('button', { name: '保存' }).click()

  // Assert
  await expect(page.getByText('共有相手を更新しました')).toBeVisible()
})
