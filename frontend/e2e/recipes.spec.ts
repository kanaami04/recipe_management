...
test('レシピを新規作成すると成功トーストが出る', async ({ page }) => {
  ...
  await page.getByPlaceholder('title').fill('新レシピ')
  await page.getByRole('combobox').first().click() // create_for
  await page.getByRole('option', { name: '2', exact: true }).click()
  // 食材・調味料それぞれ初期1行の name を埋める(material 名は必須のため)。
  await page.getByPlaceholder('name').nth(0).fill('じゃがいも')
  await page.getByPlaceholder('name').nth(1).fill('塩')
